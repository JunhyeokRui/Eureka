...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.obs_vel, self.prev_obs_vel)
...
@torch.jit.script
def compute_reward(obs_vel: torch.Tensor, prev_obs_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Compute the change in forward velocity
    delta_vel = obs_vel[:, 1] - prev_obs_vel[:, 1]
  
    # Define a reward function that rewards increasing forward velocity
    reward = torch.exp(delta_vel)
  
    # Create a dictionary to store individual reward components
    reward_components = {'delta_vel': delta_vel}
