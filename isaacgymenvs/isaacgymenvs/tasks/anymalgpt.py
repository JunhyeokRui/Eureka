...
class AnymalGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.commands, self.root_states, self.lin_vel_scale, self.ang_vel_scale)
...
@torch.jit.script
def compute_reward(commands: torch.Tensor, root_states: torch.Tensor, lin_vel_scale: float, ang_vel_scale: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # define temperature variables for reward transformation
    lin_vel_temp = torch.tensor(10.0, requires_grad=False, device=commands.device)  # Increased for more sensitivity
    ang_vel_temp = torch.tensor(0.5, requires_grad=False, device=commands.device)  # Slight decrease from default value of 1

    # calculate the difference between the desired and actual velocities in X,Y and yaw
    vel_diff = commands - root_states[:, 7:10] 

    # scale the velocity differences
    vel_diff[:, :2] *= lin_vel_scale
    vel_diff[:, 2] *= ang_vel_scale

    # square the differences and take the mean to get a single scalar reward
    reward = torch.mean(torch.square(vel_diff))

    # transform the reward using exponential function to bound it between 0 and 1
    reward = torch.exp(-lin_vel_temp * reward)

    # define and calculate the individual reward components
    reward_components = {"linear_velocity_diff": torch.exp(-lin_vel_temp*torch.mean(torch.square(vel_diff[:, :2]))),
                         "angular_velocity_diff": torch.exp(-ang_vel_temp*torch.mean(torch.square(vel_diff[:, 2])))}
